defmodule Grapple.Distributed.ReplicationEngine do
  ...
  defp get_replication_strategy(replication_policy, key, data, state) do
    base_policy = Map.get(state.replication_policies, replication_policy, 
                          state.replication_policies.balanced)
  ...
  defp adapt_replication_strategy(key, data, base_policy, state) do
    ...
    %{base_policy | 
      min_replicas: adapted_replicas,
      max_replicas: adapted_replicas
    }
  end
  ...
  defp create_replica_set(key, data, strategy, state) do
    ...
    target_nodes = case LifecycleManager.get_replication_nodes(key, strategy.max_replicas) do
      {:ok, nodes} -> nodes
      {:error, :key_not_classified} -> 
        ...
        case LifecycleManager.get_replication_nodes(key, strategy.max_replicas) do
          {:ok, nodes} -> nodes
          _ -> [node()]  # Ultimate fallback: use current node
        end
      _ -> [node()]
    end
    
    # Create replica entries
    replicas = Enum.map(target_nodes, fn node ->
      %{
        node: node,
        data: data,
        version: 1,
  ...
  defp replicate_to_remote_nodes(key, data, target_nodes, strategy) do
    ...
    Enum.each(remote_nodes, fn target_node ->
      try do
        GenServer.call({__MODULE__, target_node}, 
          {:store_replica, key, data, strategy}, 5000)
      catch
        :exit, _ -> 
  ...
  defp detect_conflicts(vector_clock_a, vector_clock_b) do
    ...
    keys = MapSet.union(MapSet.new(Map.keys(vector_clock_a)), 
                       MapSet.new(Map.keys(vector_clock_b)))
    
    comparison_results = Enum.map(keys, fn key ->
    ...
    case {Enum.member?(comparison_results, :greater), 
          Enum.member?(comparison_results, :less)} do
      {true, false} -> :no_conflict  # A > B
      {false, true} -> :no_conflict  # B > A
      {false, false} -> :no_conflict # A = B
  ...
  defp update_replica_in_set(replica_set, updated_replica) do
    updated_replicas = Enum.map(replica_set.replicas, fn replica ->
  ...
  defp propagate_update(key, updates, _vector_clock, replica_set) do
    ...
    Task.start(fn ->
      ...
      Enum.each(remote_nodes, fn target_node ->
        try do
          GenServer.call({__MODULE__, target_node}, 
            {:update_replica, key, updates, node()}, 5000)
        catch
          :exit, _ -> :ok
        end
      end)
  ...
  defp resolve_replica_conflicts(replica_set, state) do
    ...
    conflicted_replicas = Enum.filter(replica_set.replicas, fn replica ->
  ...
  defp resolve_graph_node_conflicts(conflicted_replicas, state) do
    ...
    merged_properties = conflicted_replicas
  ...
  defp resolve_graph_edge_conflicts(conflicted_replicas, state) do
    ...
    structural_consistent = Enum.all?(conflicted_replicas, fn replica ->
  ...
  defp promote_replica_to_primary(key, target_node, state) do
    case get_replica_set(key, state) do
      {:ok, replica_set} ->
        ...
        updated_replica_set = %{replica_set | 
  ...
  defp handle_node_failure_internal(failed_node, state) do
    ...
    affected_sets = state.replica_sets
  ...
  defp handle_replica_set_failure(key, replica_set, failed_node, state) do
    ...
    {new_primary, updated_replicas} = if replica_set.primary_node == failed_node do
    ...
    final_replicas = if current_count < target_replica_count and new_primary != nil do
  ...
  defp calculate_replica_health(replica_set) do
    ...
    healthy_replicas = Enum.count(replica_set.replicas, fn replica ->
      # Consider healthy if updated recently
      System.system_time(:second) - replica.last_updated < 300  # 5 minutes
    end)
  ...
  defp check_replica_set_consistency(key, replica_set) do
    ...
    data_versions = Enum.map(replica_set.replicas, fn replica ->
  ...
  defp calculate_replication_efficiency(state) do
    ...
    health_ratios = state.replica_sets
  ...
  defp calculate_conflict_rate(state) do
    total_conflicts = state.replica_sets
    |> Enum.map(fn {_key, replica_set} -> count_total_conflicts(replica_set) end)
    |> Enum.sum()
    
    total_replicas = state.replica_sets
  ...
  defp count_failover_events(state) do
    ...
    recent_threshold = System.system_time(:second) - 3600  # Last hour
