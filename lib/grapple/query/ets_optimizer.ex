defmodule Grapple.Query.EtsOptimizer do
  ...
  defp get_all_nodes_optimized do
    try do
      nodes = :ets.tab2list(:grapple_nodes)
  ...
  defp perform_optimized_traverse(start_node_id, direction, depth) do
    visited = MapSet.new([start_node_id])  # Start node is already visited
  ...
  defp traverse_bfs_optimized([{node_id, current_depth} | rest], visited, results, direction, max_depth) do
    if MapSet.member?(visited, node_id) or current_depth > max_depth do
      ...
      case EtsGraphStore.get_node(node_id) do
        {:ok, node} ->
          ...
          {new_queue, next_visited} = if current_depth < max_depth do
